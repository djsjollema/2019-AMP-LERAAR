...
if(!this.endPoint.dragging){
  ...
  clamp(point,vel){
    if (point.position.dx > canvas.width-point.radius){
      vel.dx = - Math.abs(vel.dx);
      point.point.dx = width-point.point;
    }
    if (point.position.dx < point.radius){
      vel.dx =  Math.abs(vel.dx);
      point.point.dx = point.radius
    }
    if (point.position.dy > height-point.radius){
      vel.dy = - Math.abs(vel.dy);
      point.position.dy = height-point.radius
    }
    if (point.position.dy < point.radius){
      vel.dy =  Math.abs(vel.dy);
      point.position.dy = point.radius
    }
  }
